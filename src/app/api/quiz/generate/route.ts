// app/api/quiz/generate-questions/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 입력 검증
    const validationResult = QuestionsGenerationRequestSchema.safeParse(body)
    ...
    const prompt = PromptTemplate.fromTemplate(`
      ...
      주관식 문제(subjective)의 경우:
      1. 명확한 정답을 제공하세요. 정답은 반드시 1단어로 제한하세요.
      2. options 배열은 비워두세요.
      
      문제 유형이 객관식(multiple_choice)으로 지정된 경우, 절대로 주관식 문제를 생성하지 마세요.
      또한 주관식(subjective) 문제로 지정된 경우, 절대로 객관식 문제를 생성하지 마세요.
    `)
